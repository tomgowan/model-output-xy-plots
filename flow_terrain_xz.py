...
for t in range(200,201):
    ...
    for run in range(1,7):
        ...
        #Plot Wind
        wind_plot_xz = plt.contourf(x2d[run-1,:,:], z2d[run-1,:,:]*z_scale, model_run.uinterp[t,bottom:top,ymid,left:right].values, levels, cmap = shifted_cmap, extend = 'both', alpha = 1,  zorder = 3)
...
#    plt.quiverkey(quiv, X=-1.65, Y=-0.2, U=20, linewidth = 0.75, color = 'k', label='20 $\mathregular{ms^{-1}}$', labelpos='W', fontproperties={'size': '28'})
    ...
    #Save and Close
    plt.savefig("/uufs/chpc.utah.edu/common/home/u1013082/public_html/phd_plots/cm1/plots/flow_terrain_xz_{:03d}.png".format(t), dpi = 150)
    plt.close(fig)  
