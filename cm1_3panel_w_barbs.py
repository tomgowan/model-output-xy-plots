...
#weak wind
ds_weak = xr.open_dataset('/uufs/chpc.utah.edu/common/home/steenburgh-group7/tom/cm1/cm1r19/run/old_runs/cm1out_200m_weak_wind.nc')

#mod wind
ds_mod = xr.open_dataset('/uufs/chpc.utah.edu/common/home/steenburgh-group7/tom/cm1/cm1r19/run/old_runs/cm1out_200m_normal_wind.nc')

#string wind
ds_strong = xr.open_dataset('/uufs/chpc.utah.edu/common/home/steenburgh-group7/tom/cm1/cm1r19/run/old_runs/cm1out_200m_strong_wind.nc')
...
for i in range(35,len(ds_weak.u[:,0,0,0])):
    ...
    for j in range(1,4):
        ...
        barbs = ax.barbs(y, x, U[points], V[points], length = 6, zorder = 4)
        
        #Plot variable details
        levels_cref = np.arange(-1.5,1.501,.05)
        levels_ticks = np.arange(-1.5,1.501,.2)
        cref_plot = plt.contourf(model_run.w[i,6,:,:], levels_cref, cmap = cm.RdBu, alpha = 1, extend = 'both', zorder = 3) #vmin = -10,
    
        #Water and land details
        levels_water = [1.5, 2.5]
        levels_terrain = [0,1.5]
        terrain_levels = np.arange(0, 800.1, 5)
        terrain_ticks = np.arange(0,800.1,100)
        xland_plt = plt.contourf(ds_weak.xland[0,:,:], levels_water, alpha = 1, colors = ('lightsteelblue'), zorder = 2)
        xland_plt = plt.contour(ds_weak.xland[0,:,:], levels_terrain, alpha = 1, colors = ('k'), zorder = 10)
        ...
        if j == 1:
            ...
            plt.title("Vertical Velocity at 1-km [m/s] (elapsed time = %d seconds)"  % secs, fontsize = 22, y = 0.98)
    
    
    
    #Colorbar
    cbaxes = fig.add_axes([0.91, 0.2, 0.035, 0.55])             
    cbar = plt.colorbar(cref_plot, cax = cbaxes, ticks = levels_ticks)
    cbar.ax.tick_params(labelsize=12)
    
    
    plt.savefig("/uufs/chpc.utah.edu/common/home/u1013082/public_html/phd_plots/cm1/varied_wind_verticalvelocity%03d.png" % i)
