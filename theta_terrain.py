...
for t in range(200,201):
    ...
    for run in range(1,7):
        ...
        #Plot U-Wind
        theta = model_run.th[t,t_xy+9,near:far,left:right].values-model_run.th[0,t_xy+9,near:far,left:right].values #30 minute change in theta
        wind_plot_xy = ax.contourf(theta, levels, cmap = shifted_cmap, extend = 'both', alpha = 1,  zorder = 3)
        
        #Plot W-Wind
        w_wind_int = model_run.winterp[t-10:t+10,t_xy+1:t_xy+7,near:far,left:right].mean(dim = ['time', 'nk']).values
        ...
        #Plot Lake
        water = plt.contour(model_run.xland[0,near:far,left:right], levels_water, alpha = 1, colors = ('blue'), zorder = 4, linewidths = 4)
        
        #Plot Terrain
        try:
            terrain = plt.contour(model_run.zs[0,near:far,left:right], terrain_levels, alpha = 1, colors = 'w', vmin = -800, vmax = 3000, zorder = 4, linewidths = 1.75)
    ...
    plt.text(0.35, -1.85, "${\Theta}$' (K)", fontsize = 24)
    
    #Quiver
    plt.quiverkey(quiv, X=-1.65, Y=-0.2, U=20, linewidth = 0.75, color = 'k', label='20 $\mathregular{ms^{-1}}$', labelpos='W', fontproperties={'size': '28'})
    
    #Save and Close
    plt.savefig("/uufs/chpc.utah.edu/common/home/u1013082/public_html/phd_plots/cm1/plots/theta_terrain_{:03d}.png".format(t), dpi = 70)
    plt.close(fig)  
