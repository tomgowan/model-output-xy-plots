...
for i in range(0,len(ds_weak.v[:,0,0,0])):
    ...
    for j in range(1,4):
        ...
        levels_cref = np.arange(-8,8.01,0.1)
        levels_ticks = np.arange(-8,8.01,1)
        cref_plot = plt.contourf(model_run.v[i,0,:,:], levels_cref, cmap = cm.RdBu, alpha = 1, extend = 'both', zorder = 3) #vmin = -10,
        ...
        xland_plt = plt.contour(ds_weak.xland[0,:,:], levels_terrain, alpha = 1, colors = ('white'), zorder = 5)
        ...
        if j == 1:
            #Title
            plt.title("V-Component Surface Wind [m/s] (elapsed time = %d seconds)"  % secs, fontsize = 22, y = 0.98) 
    ...
    plt.savefig("/uufs/chpc.utah.edu/common/home/u1013082/public_html/phd_plots/cm1/png_for_gifs/varied_wind_v%03d.png" % i)
